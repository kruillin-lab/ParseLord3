...
internal static class DataCenter
{
    ...
    /// <summary>
    /// When true, the player has manually selected a target and ParseLord should not override it.
    /// This is set when the player changes target via game input (not via ParseLord auto-targeting).
    /// </summary>

    public static bool ManualTargetOverride { get; set; } = false;

    /// <summary>
    /// The GameObjectId of the target when ManualTargetOverride was set.
    /// Used to detect when the manually selected target dies or becomes invalid.
    /// </summary>
    public static ulong ManualTargetId { get; set; } = 0;

    /// <summary>
    /// Timestamp when the manual target override was set.
    /// Can be used for timeout logic if desired.
    /// </summary>
    public static DateTime ManualTargetTime { get; set; } = DateTime.MinValue;
    ...
    /// <summary>
    /// Clears the current command action from the queue (used when CanUse fails).
    /// </summary>
